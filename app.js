...
function init() {
  ...
  // 【超重要】Firebaseの同期完了を待たずに、まずローカルデータで今日のやることタスクを生成して描画する！
  // これにより、起動直後に画面が白紙になるのを100%防ぎます。
  try {
    checkDateChange();
...
function startLocalMode() {
  ...
  cloudDataLoaded = true; // ローカルモードのロード完了のため、ガードを解除！
}
...
async function loadCloudData() {
  ...
  try {
    ...
    try {
      ...
      if (doc.exists) {
        const data = doc.data();
        console.log("[Firestore Sync] Realtime cloud data update detected:", data);
...
function repairTodayCompletedTasks() {
  ...
  try {
    const todayDay = typeof getTodayDayName === 'function' ? getTodayDayName() : "日";
    const todayDateStr = typeof getTodayDateString === 'function' ? getTodayDateString() : new Date().toLocaleDateString('ja-JP');
    const todaySchedules = gameState.weeklySchedules 
      ? gameState.weeklySchedules.filter(s => s && s.days && Array.isArray(s.days) && s.days.includes(todayDay)) 
      : [];

    todaySchedules.forEach(schedule => {
      try {
        if (!schedule || schedule.drillId) return; // ドリルは別の自動生成ロジックで制御されるためスキップ

        const schedName = String(schedule.name || "").trim();
        if (!schedName) return; // 名前が空ならスキップ

        const weeklyTaskId = `weekly_${schedule.id || Date.now()}_${todayDateStr}`;
        
        // 今日すでにこの予定が完了実績（completedTasks）にあるか？
        const isCompleted = Array.isArray(completedTasks) && completedTasks.some(t => {
          try {
            if (!t) return false;
            const tText = String(t.text || "").trim();
            return tText === schedName && (t.completedDate === todayDateStr || t.date === todayDateStr);
          } catch (e) {
            return false;
          }
        });
        ...
        console.error("Error healing single weekly schedule task:", err);
      }
    });
  } catch (err) {
    console.error("Error during weekly schedules healing process:", err);
  }
...
function generateDailyTasks(isNewDay = false) {
  try {
    ...
    console.error("repairTodayCompletedTasks failed in generateDailyTasks:", e);
  }
  ...
  if (Array.isArray(tasks)) {
    tasks.forEach(t => {
      if (t && t.date === todayDateStr && t.status === 'completed' && t.drillId) {
        completedDrillIdsToday.add(t.drillId.toString());
      }
    });
  }
  ...
  if (Array.isArray(tasks)) {
    ...
    tasks = tasks.filter(t => {
      ...
      if (t.date === todayDateStr && t.status === 'active' && t.drillId && completedDrillIdsToday.has(t.drillId.toString())) {
        return false;
      }
  ...
  tasks = tasks.filter(task => {
    ...
    if (task.drillId !== null && task.drillId !== undefined) {
      return activeDrillIdsToday.has(task.drillId.toString()) || task.status === 'completed' || task.status === 'postponed' || task.status === 'deleted';
...
function renderTasks() {
  ...
  // 【超強力・WebKitチェックマークバグ対策】
  // HTML の checked 属性がブラウザのキャッシュやProperty状態によって無視されるのを防ぐため、
  // 生成された実際のチェックボックスDOMに対して直接 checked プロパティを書き込みます。
  const checkboxes = taskListEl.querySelectorAll('.task-checkbox');
  checkboxes.forEach(cb => {
    const tId = cb.dataset.id;
    const t = tasks.find(task => task.id === tId || task.id.toString() === tId);
...
function setupEventListeners() {
  ...
  // (iPadOS Safari無限ループ回避のためタップリスナーは削除)
...
// 新しいタスク追加用プルダウンの生成 (iPadOS Safariの無効属性バグを回避したフラットリスト)
function renderNewTaskDrillOptions() {
  ...
  // 1. ドリル・宿題の追加（例外安全ガード）
  try {
    const activeDrills = Array.isArray(drills) ? drills.filter(d => d && !d.archived) : [];
    activeDrills.forEach(drill => {
      try {
        const opt = document.createElement('option');
        opt.value = `drill:${drill.id}`;
        const catLabel = categoryMap[drill.category] || drill.category || "勉強";
        const drillNameStr = String(drill.name || "名称未設定").trim();
        opt.textContent = `📚【ドリル】${drillNameStr} (${catLabel})`;
        selectEl.appendChild(opt);
      } catch (err) {
        console.error("Error adding drill option:", err);
      }
    });
  } catch (err) {
    console.error("Error processing drills filter:", err);
  }

  // 2. 時間割の予定（習い事等）の追加（例外安全ガード）
  const uniqueLessons = [];
  const addedLessonNames = new Set();
  
  try {
    if (gameState && gameState.weeklySchedules && Array.isArray(gameState.weeklySchedules)) {
      gameState.weeklySchedules.forEach(s => {
        try {
          if (s && !s.drillId && s.name) {
            const nameStr = String(s.name).trim(); // 数値などの型揺れも安全に文字列へキャスト
            if (nameStr && !addedLessonNames.has(nameStr)) {
              addedLessonNames.add(nameStr);
              uniqueLessons.push(s);
            }
          }
        } catch (err) {
          console.error("Error processing single weekly schedule item:", err);
        }
      });
    }
  } catch (err) {
    console.error("Error processing weeklySchedules:", err);
  }
  ...
  try {
    uniqueLessons.forEach(schedule => {
      try {
        ...
        const catLabel = categoryMap[schedule.category] || schedule.category || "予定";
        const scheduleNameStr = String(schedule.name || "名称未設定").trim();
        opt.textContent = `🏆【予定】${scheduleNameStr} (${catLabel})`;
        selectEl.appendChild(opt);
      } catch (err) {
        console.error("Error adding schedule option:", err);
      }
    });
  } catch (err) {
    console.error("Error processing uniqueLessons:", err);
  }
